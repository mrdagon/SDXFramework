...
<HR>
<B>リソースファイル名</B>\n
エラーが出るため、画像やテキストファイル名、フォルダ名等はアルファベット(a~z,A~Z)と数字(0~9)と半角アンダーバーのみにして下さい。\n
<HR>
<B>WindowsXP</B>\n
...
<HR>
<B>識別子</B>\n
Androidでは関数名やクラス名等はアルファベット(a~z,A~Z)と数字(0~9)と半角アンダーバー以外を使うとエラーが出る事があります。\n
仮引数や変数に使う分にはエラーは出ないようです。Android以外では問題なく英語以外の識別子が使えます。\n
...
複数のOSに対応する場合、OS標準フォントは使うべきではありません\n
<HR>
<B>DXLIB版とSDL版</B>\n
