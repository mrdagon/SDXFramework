...
　数ページの規約を守れば誰でも分かりやすいコードが書けるなんてことはないです。\n
 ...
 　なので本規約は「複数のコーディングスタイルがありどちらも大した差がないので、どちらかに統一する｣事が主眼に置いています。\n
  ...
  　また特殊な事情で制限をかけている事が少しあるので、その辺りの注意等も書いています\n
   ...
   InitializeをInitとしたりしているが、極力さけたいので今の内に変更するかもしれない\n
   \n
   <B>ファイル名</B>\n
   ...
   　基本的に日本語にします。\n
	...
	\n
	<B>慣用的な命名</B>\n
	ループカウンタはa,b,cの順で一文字の変数を用いる、3重以上のループは書いていない\n
	...
	@section その他
	<B>著作権表示</B>\n
	コードの先頭には著作権表示をコメントする\n
	//©SDXFramework http://sourceforge.jp/projects/dxframework/
	\n
	　ライブラリのコードなのかユーザーが書いたコードなのか見分けをつけやすくするのと、\n
	 権利を主張している事を明示的にするため必要です\n
	 \n
	 機種依存文字(というかフォント依存文字)の©を使うのは、MSVCでUTF-8で確実に保存するため\n
	 <B>名前空間</B>\n
