...
@page CodingRule SDXコーディング規約
 \n
@section はじめに
　数ページの規約を守れば誰でも分かりやすいコードが書けるなんてことはありません。\n
例えば、「yes/noを表す変数はintで無くboolを使おう｣とかは規約で決めなくても普通に考えれば皆そうします。\n
初心者だらけの開発チームでも無い限り、常識的な事を規約に書くのは逆効果です\n
\n
なので本規約は「複数のコーディングスタイルがありどちらも大した差がないので、どちらかに統一する｣と言った所を主眼に置いています。\n
例えば、インデントを4か8にするかは大した差が無いかもしれませんが、少なくとも同じプロジェクト内ではどちらかに統一すべきです\n
\n
<B>基本方針</B>\n
①C++初学者のために初歩的な構文だけ使うとか考えない\n
②最適化に頼れる所は頼り、無駄な最適化の作業は避ける\n
...
InitializeをInitとしたりしているが、極力さけよう\n
\n
<B>ファイル名</B>\n
...
ローマ字表記は避け、英単語を元に命名します、$記号は使ってはいけません\n
\n
<B>大文字と小文字の使い分け</B>\n
...
\n
<B>前置インクリメント</B>\n
最適化で速度的に差はないと思われるが、デバッグビルドもあるので前置インクリメントを使う\n
どっちにしろ差は殆ど無いはず\n
\n
<B>float型</B>\n
基本的に使わず、doubleに統一する\n
